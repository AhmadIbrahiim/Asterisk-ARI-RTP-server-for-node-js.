...
export class AriServer extends EventEmitter {
  ...
  private setupEventHandlers(): void {
    this.client.on("StasisStart", async (event: any) => {
      ...
      try {
        ...
        await this.answer(channelId)
        await this.playSound(channelId, "sound:en/custom/intro")
    ...
    this.client.on("StasisEnd", async (event: any) => {
      const channelId = event.channel.id
      console.log("Channel left Stasis:", channelId)

      try {
        // Only cleanup the specific channel
        await this.hangup(channelId).catch((err: Error) => {
          // Ignore hangup errors as the channel might already be gone
          console.log(`Channel ${channelId} already hung up:`, err.message)
        })

        // Remove channel from bridge if it exists
        if (this.bridge) {
          await this.client.bridges.removeChannels(this.bridge.id, { channel: channelId }).catch((err: Error) => {
            // Ignore remove errors as the channel might already be gone
            console.log(`Channel ${channelId} already removed from bridge:`, err.message)
          })
        }
      ...
      console.log("Application still running, waiting for new calls...")
    })
  ...
  public async answer(channelId: string): Promise<void> {
    ...
    try {
      await axios.post(`${this.baseUrl}/channels/${channelId}/answer`, {}, { auth: this.auth })
  ...
  public async hangup(channelId: string): Promise<void> {
    ...
    try {
      await axios.delete(`${this.baseUrl}/channels/${channelId}`, { auth: this.auth })
  ...
  public async playSound(channelId: string, sound: string): Promise<void> {
    if (!this.isConnected) {
      throw new Error("ARI client is not connected")
    }

    try {
      const channel = this.client.channels.Channel({ id: channelId })
      await channel.play({ media: sound })
      console.log(`Playing sound ${sound} on channel ${channelId}`)
    } catch (error) {
      console.error(`Failed to play sound on channel ${channelId}:`, error)
      throw error
    }
  }
}
